...
For example if you send an input value of 0.1 to the muscle model a small
number of weak motor units will be recruited and a small fraction of the total
possible force will be generated by the muscle. If, however, you use an value
of 0.95 then all or nearly all the motor units will be recruited and the muscle
will produce close to maximal voluntary output.

Output values are normalized by the maximum voluntary force the muscle can
produce. Output values will thus be between 0.0 and 1.0. The amount
of force a muscle can produce is determined mainly by the gain value on
the muscle actuator set in each model.
